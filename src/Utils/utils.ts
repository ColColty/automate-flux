...
export function toCamelCase(str: string): string {
    return str.replace(/(?:^\w|[A-Z]|\b\w|_\w)/g, function (word, index) {
...
export async function getSendProperty(propertiesSend: ParsedProperty[]): Promise<ParsedProperty[]> {
    ...
    const resOptional = await vscode.window.showQuickPick([
        "Yes",
        "No"
    ], {
